...
const NewPlaceForm = () => {
    ...
    const { token } = UseAuth();
    const [data, setData] = useState({ title: "", address: "", description: "", extraInfo: "", checkIn: "", checkOut: "", maxGuests: "" })
    const handleInput = (e) => {
    ...
    const AddData = async (e) => {
        e.preventDefault();
        const { title, address, description, extraInfo, checkIn, checkOut, maxGuests } = data;
        ...
        try {
            ...
            if (response.ok) {
                setData({ title: "", address: "", description: "", extraInfo: "", checkIn: "", checkOut: "", maxGuests: "" })
                ...
                <form className="flex flex-col gap-4" onSubmit={AddData}>
                    ...
                    <div className="flex flex-row gap-4 justify-center items-center flex-wrap">
                        <div>
                            ...
                            <input type="date" id="checkIn" className="border-2 border-gray-400 px-4 py-3 rounded-2xl w-44  borderDefault" name="checkIn" value={data.checkIn} onChange={handleInput} />
                        </div>
                        <div>
                            <label htmlFor="checkOut" className="mx-3 text-lg">CheckOut</label>
                            <p className="mx-3 text-sm text-gray-500">enter checkOut time</p>
                            <input type="date" id="checkOut" className="border-2 border-gray-400 px-4 py-3 rounded-2xl w-44  borderDefault" name="checkOut" value={data.checkOut} onChange={handleInput} />
                        </div>
                        <div>
